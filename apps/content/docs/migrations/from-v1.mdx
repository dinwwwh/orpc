...
The index-based validation options were replaced by two simple flags. v2 tracks the order of middleware and validation automatically, so `dedupeLeadingMiddlewares` and the index options are gone. See [Validation Customization](/docs/recipes/validation-customization).
...
Guard shared middleware yourself with the context flag pattern from [Dedupe Middleware](/docs/recipes/dedupe-middleware):
...
`rootInterceptors` is now `routingInterceptors`. `adapterInterceptors` was removed, because regular interceptors can now customize body parsing behavior. See [RPC Handler](/docs/rpc/handler).
...
const serializer = new RPCSerializer({
  handlers: {
    user: {
      ...
      deserialize: (data: any) => new User(data.id, data.name),
    },
  },
})
...
### `RegExp` is no longer a built-in type

The [RPC Serializer](/docs/rpc/serializer) and [OpenAPI Serializer](/docs/openapi/serializer) no longer serialize `RegExp` values, and the [Smart Coercion Plugin](/docs/plugins/smart-coercion) no longer recognizes `x-native-type: 'regexp'`. A `RegExp` now serializes like any other class instance, as an empty object. If you send regular expressions over the wire, register a handler to restore the previous behavior:
...
const serializer = new RPCSerializer({
  handlers: {
    regexp: {
      condition: data => data instanceof RegExp,
      serialize: (data: RegExp) => data.toString(),
      deserialize: (data: string) => {
        const [, pattern, flags] = data.match(REGEXP_PATTERN)!
...
`clientInterceptors` is now `transportInterceptors`. `adapterInterceptors` was removed, mirroring the handler-side change. Event stream options moved under `toFetchRequest.eventStream`. See [RPC Link](/docs/rpc/link).
...
The implementer works the same as in v1: `implement(contract)` still supports `.$context`, `.use`, and `.middleware`. New in v2, it also accepts a [procedure config](/docs/recipes/validation-customization), either as a second argument or through `.$config`. See [Contract Implementation](/docs/contract/implementation).
