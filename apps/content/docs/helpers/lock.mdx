...
By default every key gets its own Durable Object. An object serves any number of keys independently, each tracked as a WebSocket tag, so to bound how many objects exist you can map keys onto a fixed set of them with `getStubByName` without changing how the locks behave. Keys are limited to 256 characters.
