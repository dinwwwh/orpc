...
Stores locks in Durable Objects, one per key by default, so every Worker instance shares the same locks. Callers hold a hibernatable WebSocket while holding or waiting, so objects keep no storage and are not billed meanwhile. Export a class extending `DurableLockObject`, bind it in your Wrangler config, and pass the namespace to `DurableLocker`.
...
An object serves any number of keys independently, so `getStubByName` can map keys onto a fixed set of objects. Keys are limited to 256 characters.
