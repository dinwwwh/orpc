...
An object serves any number of keys independently, so `getStubByName` can map keys onto a fixed set of objects.

```ts
const locker = new DurableLocker(env.LOCK_DON, {
  ttl: 30_000,
  getStubByName: (namespace, key) => namespace.getByName(key.slice(0, 2)),
})
```
