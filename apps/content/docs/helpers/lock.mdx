...
The `ttl` option sets how long the lock is held before it expires automatically, in milliseconds. It guards against holders that never release the lock, such as a crashed process. Each adapter has its own default:
