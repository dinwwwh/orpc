...
`ttl` sets how long the lock is held before it expires automatically, in milliseconds. It guards against holders that never release the lock, such as a crashed process. Each adapter has its own default:

```ts
await locker.lock('report:123', () => generateReport('123'), { ttl: 30_000 })
```

`timeout` sets how long to wait for the lock to become available, in milliseconds. When it elapses, `lock` rejects with a `LockTimeoutError`. Set `timeout: 0` to give up immediately when the lock is held by someone else, which turns the lock into a "skip if already running" guard:
...
`signal` stops waiting early. It rejects with the abort reason and has no effect once the lock is acquired:

```ts
await locker.lock('report:123', () => generateReport('123'), { signal: request.signal })
```
