...
Keeps each lock in a Durable Object named after its key, so every Worker instance shares the same locks. Waiting callers park on a hibernatable WebSocket instead of polling, so the object is not billed while they wait and the lock is handed over the moment it is released. Export a class extending `DurableLockObject` and bind it as a Durable Object namespace in your Wrangler config, then pass the namespace to `DurableLocker`.
...
const locker = new DurableLocker(env.LOCK_DON, {
  ttl: 30,
  getStubByName: (namespace, key) => namespace.getByName(`stripe:${stripe(key)}`),
})
```
