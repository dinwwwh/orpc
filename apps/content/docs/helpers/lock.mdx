...
- `timeout`: how long to wait for the lock to become available, in milliseconds. `lock` rejects with a `LockTimeoutError` once it elapses, and `0` gives up immediately when the lock is held by someone else.
