...
Keeps each lock in a Durable Object named after its key, so every Worker instance shares the same locks. Export a class extending `DurableLockObject` and bind it as a Durable Object namespace in your Wrangler config, then pass the namespace to `DurableLocker`.
