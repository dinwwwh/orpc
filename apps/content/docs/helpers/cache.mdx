...
import { MemoryLock } from '@orpc/shared'
...
const store = new VercelCacheStore({
  ...
  /**
   * Coalesces concurrent callers of one key. The Runtime Cache has no
   * atomic primitive, so the default holds the lock within the process.
   *
...
The `cache` helper creates middleware that caches the output of [procedures](/docs/procedure). On a hit it returns the cached output without executing the handler, and on a miss it executes the handler and stores the result. Concurrent misses for one key run the handler once and the rest are served from the entry it stores. The `key`, `tags`, `ttl`, `swr`, and `enabled` options accept static values or functions of the middleware options and input.
