...
const serializer = new RPCSerializer({
  handlers: {
    person: { // <- add support for Person
      ...
      deserialize: (v) => {
        const { name, age } = PersonSchema.parse(v)
    ...
    date: { // <- replace the default Date handler
      ...
      deserialize: v => new Date(z.number().parse(v)),
    },
  },
})
```
...
:::warning
`deserialize` receives untrusted input. A malicious client controls `meta` and can point any handler at any JSON value, so never assume `deserialize` receives what `serialize` produced. Validate the value and throw when it does not match. Built-in handlers do the same, and [RPC Handler](/docs/rpc/handler) turns these errors into a `BAD_REQUEST` response.
:::
