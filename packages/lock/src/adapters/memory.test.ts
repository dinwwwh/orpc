...
describe('memoryLocker', () => {
  it('runs the callback immediately when the lock is free', async () => {
    ...
    expect((locker as any).entries.size).toBe(0)
  })

  it('waits for the holder to release and serves waiters in order', async () => {
    const locker = new MemoryLocker()
  ...
  it('tracks locks independently per key', async () => {
    ...
    await expect(locker.lock('bob', fn)).resolves.toBe('bob')
    expect(fn).toHaveBeenCalledWith({ waited: false })
  ...
  describe('timeout', () => {
    ...
    it('rejects with LockTimeoutError when the lock is not released in time', async () => {
      ...
      expect((locker as any).entries.get('key').waiters).toHaveLength(0)
